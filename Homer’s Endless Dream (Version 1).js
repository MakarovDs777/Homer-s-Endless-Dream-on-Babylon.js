  ...
  asteroids.push({asteroid: asteroid, vx: vx, vy: vy, vz: vz, radius: radius});
    ...
    scene.registerBeforeRender(function () {
        ...
        for (var i = 0; i < asteroids.length; i++) {
            ...
            // Проверяем, не вышел ли астероид за пределы области
            if (asteroid.asteroid.position.x + vx < 0 || asteroid.asteroid.position.x + vx > 100) {
                vx *= -1;
            }
            if (asteroid.asteroid.position.y + vy < 0 || asteroid.asteroid.position.y + vy > 10) {
                vy *= -1;
            }
            if (asteroid.asteroid.position.z + vz < 0 || asteroid.asteroid.position.z + vz > 100) {
                vz *= -1;
            }

            // Обновляем позицию астероида
            asteroid.asteroid.position = new BABYLON.Vector3(
                asteroid.asteroid.position.x + vx,
                asteroid.asteroid.position.y + vy,
                asteroid.asteroid.position.z + vz
            );
